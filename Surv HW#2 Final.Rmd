...
plot(flood_w, type='cumhaz', ci=TRUE, conf.int = FALSE, las=1, bty='n',
...
flood_simple <- survfit(Surv(time=hour, event=flood)~1, data=flood_only)
flood_simple_data <- summary(flood_simple)
...
ggplot() +
  ...
  geom_line(aes(x=flood_simple_data$time, y=flood_simple_data$cumhaz, color = 'Actual'), linewidth=1) + 
...
loglikes <- tibble(
  ...
  log_likelihood = c(like.e, like.w, like.g, like.log))
```
...
impact_df <- diff_df %>%
  ...
  mutate(upgrade = case_when(
    max_impact == backup ~ 'backup',
    max_impact == servo ~ 'servo',
    .default = NA)
  )
